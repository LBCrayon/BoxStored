            ...
            if key_event.keystate == key_event.key_up:
                if key_event.keycode == 'KEY_ENTER':
                    if qr_code_data:
                        ...
                        data5 = Box1  # Ignore empty scans
                        print("Scanned QR Code:", qr_code_data)
                    if data5 == Box1:
                        print("Box unlocked successfully!")
                        lcd_string("Status: Unlocked", LCD_LINE_1)
                        lcd_string("TM <3", LCD_LINE_2)
                        time.sleep(1)
                        unlock_door()
                        lcd_string("Status: Locked", LCD_LINE_1)
                        lcd_string("TT <3", LCD_LINE_2)
                    else:
